...
NANOTEMPER_RANGE = NANOTEMPER_START_COLUMN + str(COMPOUND_START_ROW) + ":" + NANOTEMPER_END_COLUMN + str(
    COMPOUND_END_ROW)

RELEVANT_ROWS = slice(COMPOUND_START_ROW - 1, COMPOUND_END_ROW)
RELEVANT_COLS = [libscol.scol2int(GLUCOSE_10_COLUMN),
                 *range(libscol.scol2int(NANOTEMPER_START_COLUMN), libscol.scol2int(NANOTEMPER_END_COLUMN) + 1)]
...
def get_kmeans_results(data, K, random_state=0):
    X = data.dropna()
    kmeans = cluster.KMeans(random_state=random_state) if K == None else cluster.KMeans(n_clusters=K,
    ...
    labeled_data = pd.concat([data, pd.Series(kmeans.labels_, index=X.index, dtype='Int64', name='kmeans cluster')],
                             axis=1)
...
def get_results_for_spreadsheet(spreadsheet, centers, labeled_data, K, random_state):
    ...
    values = [[['K = ' + str(K)]], [['Random state = ' + str(random_state)]], [[centers.to_string()]],
...
if __name__ == '__main__':
    ...
    for K in args.K:
        ...
        results_for_spreadsheet = get_results_for_spreadsheet(spreadsheet, centers, labeled_data, K, args.random_state)
        if args.update_spreadsheet:
            spreadsheet.batch_update_values(spreadsheet.SPREADSHEET_ID, "USER_ENTERED", results_for_spreadsheet)
        else:
            display_to_terminal(relevant, labeled_data, centers, results_for_spreadsheet)
