...
def get_apo_form(uniprot_accession):
    ...
    if data.get("data", {}).get("entries"):
        apo_form = sorted(data["data"]["entries"], key=lambda x: x['rcsb_accession_info']['initial_release_date'])[0]
...
if __name__ == "__main__":
    ...
    if apo_pdb_id:
        print(f"The apo form of the protein with UniProt accession {UNIPROT_ACCESSION} is: {apo_pdb_id}")
    else:
        print(f"No apo form found for the protein with UniProt accession {UNIPROT_ACCESSION}")
